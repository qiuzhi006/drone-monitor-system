...
def create_complete_map(lat_a, lon_a, lat_b, lon_b, obstacles, flight_height, safe_radius, waypoints):
    ...
    folium.PolyLine(
        ...
        color='gray', weight=3, opacity=0.5, dash_array='5,5', tooltip='原始航线'
    ).add_to(m)
    folium.PolyLine(
        locations=[(p[1], p[0]) for p in waypoints],
        color='red', weight=5, opacity=0.8, tooltip='规划航线'
    ).add_to(m)
    ...
    folium.Marker([lat_a, lon_a], popup='起点A', icon=folium.Icon(color='green', icon='play', prefix='fa')).add_to(m)
    folium.Marker([lat_b, lon_b], popup='终点B', icon=folium.Icon(color='red', icon='flag-checkered', prefix='fa')).add_to(m)
    for obs in obstacles:
        polygon_coords = [[coord[1], coord[0]] for coord in obs["coords"]]
        folium.Polygon(
            locations=polygon_coords, color='orange', fill=True, fill_color='orange',
...
if st.session_state.page == "航线规划":
    ...
    with st.sidebar:
        st.divider()
        st.header("🎮 坐标系设置")
        coord_system = st.selectbox("输入坐标系", ["GCJ-02 (高德/腾讯)", "WGS-84 (GPS)"], index=0 if "GCJ-02" in st.session_state.coord_system else 1)
        ...
        st.divider()
        st.header("🔄 避障策略")
        strategy = st.radio("选择绕行方式", options=['direct', 'left', 'right', 'best'],
                            format_func=lambda x: {'direct': '直接飞', 'left': '向左绕行', 'right': '向右绕行', 'best': '最佳航线'}[x],
        ...
        with col1:
            if st.button("💾 保存障碍物", use_container_width=True): save_obstacles()
        with col2:
            if st.button("📂 加载障碍物", use_container_width=True): load_obstacles()
    ...
    waypoints = calculate_avoidance_waypoints(start, end, st.session_state.obstacles, flight_height, safe_radius, strategy, bypass_offset)

    m_complete = create_complete_map(lat_a_display, lon_a_display, lat_b_display, lon_b_display,
...
# ==================== 飞行监控页面 ====================
    ...
    with st.sidebar:
        ...
        st.divider()
        st.subheader("📋 航线信息")
        st.caption(f"起点: {st.session_state.coords_a['lat']:.6f}, {st.session_state.coords_a['lon']:.6f}")
        st.caption(f"终点: {st.session_state.coords_b['lat']:.6f}, {st.session_state.coords_b['lon']:.6f}")
        ...
        if st.session_state.flight_sim_running:
            ...
            p2 = waypoints[min(current_index+1, len(waypoints)-1)]
            ...
            battery_remaining = max(0, 100 * (1 - min(elapsed_time, 1800) / 1800))
            ...
            current_lng, current_lat = waypoints[0][0], waypoints[0][1]
            flown_distance = 0
        ...
        with col_map:
            ...
            if st.session_state.flight_sim_running and flown_distance > 0:
                ...
                total_check = 0
                for i, seg in enumerate(seg_dists):
                    total_check += seg
                    if total_check <= flown_distance:
                        flown_pts.append(waypoints[i+1])
                    else:
                        flown_pts.append((current_lng, current_lat))
                        break
                if len(flown_pts) >= 2:
                    folium.PolyLine(
            ...
            for obs in st.session_state.obstacles:
                folium.Polygon(
                    locations=[[c[1], c[0]] for c in obs["coords"]],
        ...
        with col_panel:
            ...
            if trigger_blink:
                ...
                #blink-overlay {
                    ...
                    z-index: 9999;
                    pointer-events: none;
                    animation: fullscreen-blink 0.3s ease-in-out 3;
                }
                </style>
            ...
            completed_wp = min(current_index + 1, total_wp) if st.session_state.flight_sim_running else 0
            st.metric("当前航点", f"{completed_wp}/{total_wp}")
